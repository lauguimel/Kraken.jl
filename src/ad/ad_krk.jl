...
function _krk_sensitivity_rheology_param(setup::SimulationSetup, model::Symbol,
                                         key::Symbol, label::AbstractString)
    for rs in setup.rheology
        rs.model === model || continue
        haskey(rs.params, key) && return Float64(rs.params[key])
    end
    # fall back to a Physics/Define value of the same name
    value = _krk_sensitivity_optional_numeric(setup, (key,))
    value !== nothing && return value
    throw(ArgumentError(
        "Sensitivity dispatch: missing $label. Provide it in a " *
        "`Rheology oldroyd_b { $key = ... }` block or in Physics/Define."))
end

function _krk_sensitivity_polymer_drag_kwargs(setup::SimulationSetup, request)
    :viscoelastic in setup.modules || throw(ArgumentError(
        "Sensitivity dispatch: qoi=polymer_drag requires `Module viscoelastic`."))
    setup.lattice === :D2Q9 || throw(ArgumentError(
        "Sensitivity dispatch: qoi=polymer_drag currently supports D2Q9 only."))

    dom = setup.domain
    cx = _krk_sensitivity_required_numeric(setup, (:cx,), "cylinder center `cx`")
    cy = _krk_sensitivity_required_numeric(setup, (:cy,), "cylinder center `cy`")
    Fx_body = _krk_sensitivity_required_numeric(
        setup, (:Fx_body, :u_mean, :U), "body force / inlet mean `Fx_body`")
    ...
    kwargs = Dict{Symbol, Any}(
        ...
        :cx => cx,
        :cy => cy,
        :Wi => _krk_sensitivity_required_numeric(setup, (:Wi,),
                                                 "Weissenberg number `Wi`"),
        :nu_p => _krk_sensitivity_rheology_param(setup, :oldroyd_b, :nu_p,
                                                 "polymer viscosity `nu_p`"),
        :nu_s => _krk_sensitivity_rheology_param(setup, :oldroyd_b, :nu_s,
                                                 "solvent viscosity `nu_s`"),
        :Fx_body => Fx_body,
        :qoi => request.qoi,
        :wrt => request.wrt,
    )

    beta = _krk_sensitivity_optional_numeric(setup, (:beta, Symbol("β")))
    beta !== nothing && (kwargs[:beta] = beta)

    for key in (:n_substeps, :samples)
        value = _krk_sensitivity_optional_integer(setup, (key,))
        value !== nothing && (kwargs[key] = value)
    end
    for key in (:dt, :u_mean, :fwd_tol, :gmres_tol, :adjoint_tol, :epsR, :fd_h)
        value = _krk_sensitivity_optional_numeric(setup, (key,))
        value !== nothing && (kwargs[key] = value)
    end
...
function run_krk_sensitivity(setup::SimulationSetup)
    ...
    elseif request.qoi === :polymer_drag
        _krk_sensitivity_polymer_drag_kwargs(setup, request)
    else
        throw(ArgumentError(
            "unsupported qoi=$(request.qoi); supported qoi values are :drag, :nusselt and :polymer_drag"))
    end
