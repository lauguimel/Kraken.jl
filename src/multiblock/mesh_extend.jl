...
function extend_mesh_2d(mesh::CurvilinearMesh{T, AT};
    ...
    @inbounds for j in 1:Nη
        ...
        # West ghost: X_ext[n_ghost - k + 1, j_ext] = X[1, j] - k * (X[2, j] - X[1, j])
    ...
    @inbounds for i_ext in 1:Nξ_ext
        # Reference rows: j_ext = n_ghost + 1 (first interior),
        # j_ext = n_ghost + 2 (second interior), mirror for north.
        j1 = n_ghost + 1
        j2 = n_ghost + 2
