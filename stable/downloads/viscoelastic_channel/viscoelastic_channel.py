#!/usr/bin/env python3
r"""Reproduce the Kraken.jl example-21 viscoelastic (Oldroyd-B) channel figure.

Reads ``viscoelastic_channel.csv`` (columns: y, u_analytic, u_kraken) sitting
next to this script and plots the analytical Poiseuille profile with total
viscosity (line) against the Kraken LBM log-conformation solution (markers), on
the dark #1b1b1f Vitepress page background. The polymeric stress does not alter
the steady-state velocity, so the profiles coincide with the Newtonian parabola.

    python viscoelastic_channel.py               # -> viscoelastic_channel_repro.png
    python viscoelastic_channel.py -o /tmp/v.png # custom output path
...
Env (Kraken docs): conda run -n kraken-v0-3-figures python viscoelastic_channel.py
"""
...
def main():
    ...
    ap.add_argument("-c", "--csv", default=str(HERE / "viscoelastic_channel.csv"),
                    help="input CSV (default: viscoelastic_channel.csv next to this script)")
    ap.add_argument("-o", "--out", default=str(HERE / "viscoelastic_channel_repro.png"),
