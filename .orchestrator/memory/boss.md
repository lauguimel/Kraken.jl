...
## 2026-05-31 (cont.) — RELEASE-READINESS AUDIT (3 read-only Depts) on RC dev/v0.2-multiphysics @ 3d49db834

Per-axis verdict vs mandate (findings in bench/scratch/audit_{A,B,C}.md):
- **GPU/CPU agnostic = PASS**: 0 CUDA/Metal imports in src, uniform KA get_backend dispatch (~150 sites/40 files), 0 GPU-safety-contract violations across ~190 @kernel (no alloc/IO/dispatch; unsafe_trunc+clamp). No src/backend/ dir (modularity gap only). Caveats: Metal F32 R-drift (±2-3 Cd R≥30, use CUDA F64); 3D pull-brick segfault fixed 770375a2f (assert in RC).
- **2D/3D/axi = PARTIAL**: full 2D+3D only Newtonian(3D=cavity_3d+STL only)+Geometry/STL. 2D-only: VE/GNF/Multiphase. Thermal-3D exists (run_natural_convection_3d) but NOT name-dispatched (code-only). AMR 2D-validated, 3D unvalidated (14% corner Δρ). Axisymmetric PRESENT + .krk-reachable (old multiphase axi, Li-2010), unvalidated — mandate "deferred" framing is STALE.
- **LOC = FAIL**: 125 src files/46490 LOC; **14 HARD >700** + 13 soft. Worst: simulation_runner 2714, conservative_tree_streaming 2603, conservative_tree 2597, kraken_parser 2314. M1 offenders GREW (runner 1843→2714, parser 2010→2314). src/refinement/ AMR = 8 of 14 HARD (~12700 LOC).
- **Modular = FAIL**: 5/8 §6 dirs MISSING (bc, mesh, physics, lbm, backend); present units/geometry/io(flat). Code by artifact not concern; runner holds BC+geometry+drag+IO+6 drivers; parser holds parse+units+rheology+util.
- **krk-canonical = PARTIAL**: Define/Physics numeric-only (symbolic knobs wall_bc/advection_scheme/formulation unreachable); VE+thermal-3D+multiphase code-only; natural_convection preset+example absent.
- **Efficiency = UNCERTIFIED**: NO on-disk "within 2× reference" comparison vs external hand-tuned LBM; MLUPS CSVs exist (Metal/CPU) but unbenchmarked vs reference; COVERAGE.md flags "GPU perf scaling = Missing". Anti-patterns: VE drag integrator 4 D→H Array() copies/step (drivers/viscoelastic.jl:24-27) + 6 redundant copyto! in VE conformation loop.
- **§3 RheoTool contract = FAIL RC-wide**: NO benchmarks/results/rheotool_compare/ dir for ANY module (icoFoam/Clift/published substitutes used).

Recommended efficiency cert: cavity/TG BGK D2Q9 N=1024/2048 CUDA F64 Aqua-H100 vs published single-GPU LBM (waLBerla/Palabos/OpenLB) or roofline, ratio≥0.5.

**Implication**: a "polished mandate-passing public release" is premature on hygiene/completeness. Validated axes (Newt-2D, thermal-2D, geo/STL-2D+3D, AMR-2D) + docs are showcase-able honestly. Awaiting user posture decision (showcase-now / remediate-first / hybrid). Push = confidentiality-gated regardless.

**USER DECISION (2026-05-31):** (1) **REMEDIATE-FIRST** — SPLIT the 14 HARD files (runner 2714, parser 2314, AMR conservative_tree ×8, …) + create §6 dirs (bc/mesh/physics/lbm/backend) + efficiency certification benchmark + fill krk symbolic-knob gaps, ALL before any release → truly mandate-clean. (2) **SCRUBBED dedicated release branch** — prepare `release/v0.2` with a confidentiality audit of the 216+ commits, `.orchestrator` EXCLUDED, no AI mentions; show diff/plan; user confirms BEFORE any push (reproduces the release/v0.1 scrub pattern). Docs showing-off = parallel track. Prune = with confirmation. Drafting KRK-SHIP-002 (v0.2-hardening) remediation mission-graph next.

## 2026-06-01 — KRK-SHIP-002 plan drafted + 4 decisions LOCKED; remediation campaign STARTED

Plan `.orchestrator/ship-plan-v2.md` (Plan agent): Tier-1 = 18 missions (~13-16 sess), Tier-2 = ~24-32. Critical path S1→S8 + S2→K1; S1 (runner carve 2714, dispatch hub) / S10 (physics↔lbm de-fusion, needs bit-identical GPU codegen not just green) / E1 (Aqua cert) = HIGH risk. 8 fronts startable immediately (S2 parser, S3-S5 AMR, S9, E1, X1, P1); AMR S3-S6 CPU-pure mechanical-parallel.

**User decisions LOCKED:** Tier-2 full · efficiency = BOTH roofline + published number · axisym SCOPE-CUT (ADR'd in mandate §4) · §3 = literal RheoTool backfill (`benchmarks/results/rheotool_compare/<module>/`).

**Execution discipline:** remediation on `dev/v0.3-campaign` (worktree Kraken.jl-v0.3-campaign). SPLITs SEQUENCE on this branch (each edits Kraken.jl include-list → can't parallelize on same tree). Gate per SPLIT = direct `runtests.jl` 34598/7/0 (0 new), byte-identical `git mv` where possible (M-GEO-3c precedent). For Codex runtime checks use Boss normal-env Bash (Codex --ephemeral sandbox julia launcher is broken). Start de-risking with a CLEAN mechanical AMR split BEFORE the HIGH-risk S1/S10.

**Started:** first SPLIT = `src/refinement/conservative_tree_2d.jl` (2597 LOC) → ≤700 sibling files by concern, to validate the methodology + gate.

**S-AMR-SPLIT-1 DONE — GREEN (methodology PROVEN).** conservative_tree_2d.jl (2597) → 5 files (base_transfer 622, leaf_ops 603, patch_ledger 422, composite_ops 516, macroflows 434), all ≤700. Codex static split: 116 defs preserved (116==116), **concatenation byte-identical to HEAD original**, original deleted (only Kraken.jl included it), includes rewired in dep order. **Boss-run gate `runtests.jl` direct = 34598/7/0 (0 NEW)** — byte-identical relocation preserved behavior exactly. Committed. **Pattern for the 13 remaining HARD SPLITs: Codex static byte-identical relocation (sandbox has no julia) → Boss runs full runtests-direct gate → commit if 0-new.** ~20-25 min/SPLIT, serial on this branch (Kraken.jl include edits conflict). Next: S-AMR-SPLIT-2 = conservative_tree_streaming_2d.jl (2603).

**S-AMR-SPLIT-2..8 DONE — all 8 AMR HARD splits committed, FULL-SUITE GREEN.** Commits: -2 3819bead1, -3 bc70b61ea, -4 4c00416b1, -5 7196821e0, -6 b6bcccac9, -7 b95b83382, -8 321c00af2. Full runtests checkpoint after SPLIT-8 = **34598 pass / 7 pre-existing / 0 errored / 0 NEW** — validates the load-check-gated SPLIT-3..8 cumulatively; byte-identical-relocation methodology FULLY confirmed. **8 of 14 oversized files done.** Remaining 6 HARD: vof_2d 807, slbm 1036, axisymmetric 1498, kraken_parser 2314 (→ src/io/krk/ + K1 symbolic knobs), simulation_runner 2714 (→ src/bc/), boundary_rebuild 1240 (→ src/bc/). **Gate recipe proven: Codex static byte-identical split → `julia -e 'using Kraken'` load-check (~8s) → commit; full-suite checkpoint every ~6-8 splits.** PARKED at user checkpoint (how to continue: auto-grind domain files / careful parser+runner / parallel track E1-cert/P1-backfill/docs). User wants pedagogical updates [[feedback_pedagogical_checkpoints]].

**USER SCOPE REFINEMENT + SESSION END (2026-06-01, Boss context full).** vof_2d (807, multiphase) / slbm (1036, curvilinear) / axisymmetric (1498) = OUT of v0.2 scope (those physics deferred to a future release) → NOT split. v0.2 LOC-clean target reduced to: parser 2314 (→ src/io/krk/ + symbolic-knob feature), runner 2714 (→ src/bc/), boundary_rebuild 1240 (→ src/bc/). NEXT SESSION (max delegation): Track-A parser split+symbolic (FULL-suite gate — not pure relocation), Track-B runner+boundary_rebuild → src/bc/, PARALLEL efficiency-benchmark (Aqua, roofline+published) + RheoTool literal backfill + docs showing-off. **Full resume brief = `.orchestrator/resume-next-session.md`.** This session shipped: M9-M14 docs, VE-port (isolated, fold-gated), 8 AMR-family splits (full suite 34598/7/0).
