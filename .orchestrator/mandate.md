...
### M-GEO-7 — 3D sphere quantitative drag

- **Status**: **Part (a) local twin DONE — GREEN (Boss-verified Metal F32, 2026-05-31)**, committed `e06715c78`, **FF-merged to `dev/v0.3-campaign`** (HEAD `e06715c78`). Full suite **34598 pass (= 34594 + 4 GPU-twin) / 7 pre-existing / 0 errored**. The STL `.krk` sphere drag reproduces the validated analytic scaffold (`run_sphere_libb_3d`, itself vs Clift 2.6) to **0.39%**. `run_obstacle_libb_3d` now returns `Cd` (frontal-area silhouette). Test `test_sphere_stl_drag_krk.jl` is GPU-gated (CPU=parse+mask smoke). **Part (b) Aqua F64 publishable Cd = the remaining follow-on** (user chose "twin local now + Aqua F64 after").
...
- **Part (b) open — Aqua F64 publishable Cd**: the local twin proves SELF-CONSISTENCY (krk≡analytic); a publishable absolute Cd needs (i) higher R_LU to damp the registration sensitivity + (ii) a PHYSICAL reference. Reference fork still open: free-stream Clift 2.6 requires LOW blockage (large domain) OR a confined-sphere reference (OpenFOAM identical geometry; Schäfer-Turek is a cylinder, N/A). Micro-canary discipline satisfied (local twin done) → Aqua submission authorized once the reference/resolution scope is picked. Benchmark page → `docs/src/users/benchmarks/`.
