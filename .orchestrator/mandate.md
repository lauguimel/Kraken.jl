...
| 2026-06-01 | **KRK-SHIP-002 (v0.2-hardening) ADOPTED — remediate-first to mandate-clean before release.** Release-readiness audit (3 read-only Depts on RC `dev/v0.2-multiphysics` @ `3d49db834`) verdict: **PASS** only GPU/CPU-agnostic (0 CUDA/Metal imports in src, uniform KA dispatch, 0 GPU-safety-contract violations over ~190 @kernel); **FAIL** LOC (14 files HARD >700: runner 2714, parser 2314, AMR ×8), modularity (5/8 §6 dirs missing: bc/mesh/physics/lbm/backend), efficiency (no "within 2× reference" evidence), §3 RheoTool (no `rheotool_compare/` for ANY module); **PARTIAL** 2D/3D/axi + krk-canonical. Plan = `.orchestrator/ship-plan-v2.md`. **User decisions (2026-06-01):** ship bar = **Tier-2 full (~24-32 sessions)**; efficiency ref = **BOTH roofline %-peak-BW + a published single-GPU LBM number**; **axisymmetric SCOPE-CUT for v0.2** (old multiphase axi path stays in-code but NOT claimed/validated/user-facing — supersedes the stale "deferred" framing; "true axi" `dev/axisymmetric-true-lbm` = future); §3 = **literal RheoTool backfill** (`benchmarks/results/rheotool_compare/<module>/` real runs per module). | Every SPLIT gates on behavior-preservation (direct `runtests.jl` 34598/7/0, byte-identical `git mv` per M-GEO-3c). Remediation lands on `dev/v0.3-campaign`; `release/v0.2` cut SCRUBBED (confidentiality audit, `.orchestrator` excluded, user-confirmed) at the END. Audit details in `bench/scratch/audit_{A,B,C}.md` + boss.md. |
| 2026-06-01 | **v0.2 scope refinement (user) + AMR-family splits DONE.** `vof_2d` (807, multiphase), `slbm` (1036, curvilinear), `axisymmetric` (1498) = OUT of the v0.2 hygiene scope (those physics deferred to a future release) → NOT split now. v0.2 LOC-clean target reduced to: `kraken_parser` (2314 → `src/io/krk/` + symbolic-knob feature), `simulation_runner` (2714 → `src/bc/`), `boundary_rebuild` (1240 → `src/bc/`). The 8 AMR / mesh-refinement oversized files were split behavior-preserving (commits SPLIT-1..8 `73dfac420`..`321c00af2`); full suite `34598/7/0`. | Next session: parser + runner splits (FULL-suite gate; parser also gains symbolic `.krk` options) + PARALLEL efficiency-benchmark (Aqua roofline+published) + literal RheoTool backfill + docs showing-off (max delegation). Resume brief: `.orchestrator/resume-next-session.md`. Session ended on user request (Boss context full). |
| _(append)_ |                                                                     |                                                              |
