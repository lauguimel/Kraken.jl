#!/usr/bin/env python3
r"""Reproduce the Kraken.jl example-01 Poiseuille velocity-profile figure.

Reads ``poiseuille.csv`` (columns: y, u_analytic, u_kraken) sitting next to this
script and plots the analytical parabola (line) against the Kraken LBM solution
(markers), on the dark #1b1b1f Vitepress page background.

    python poiseuille.py                  # -> poiseuille_repro.png next to csv
    python poiseuille.py -o /tmp/p.png    # custom output path
...
Env (Kraken docs): conda run -n kraken-v0-3-figures python poiseuille.py
"""
...
def main():
    ...
    ap.add_argument("-c", "--csv", default=str(HERE / "poiseuille.csv"),
                    help="input CSV (default: poiseuille.csv next to this script)")
    ap.add_argument("-o", "--out", default=str(HERE / "poiseuille_repro.png"),
    ...
    ax.plot(u_ana, y, "-", color=line_c, lw=2.0, label="Analytical", zorder=1)
    ...
    ax.set_title("Poiseuille flow — $N_y = 32$")
