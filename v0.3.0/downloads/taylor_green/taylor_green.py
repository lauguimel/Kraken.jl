#!/usr/bin/env python3
r"""Reproduce the Kraken.jl example-03 Taylor-Green energy-decay figure.

Reads ``taylor_green.csv`` (columns: step, E_analytic, E_kraken) sitting next to
this script and plots the analytical exponential decay (line) against the Kraken
LBM mean kinetic energy (markers), on the dark #1b1b1f Vitepress page background.

    python taylor_green.py               # -> taylor_green_repro.png
    python taylor_green.py -o /tmp/t.png # custom output path
...
Env (Kraken docs): conda run -n kraken-v0-3-figures python taylor_green.py
"""
...
def load_csv(path: pathlib.Path):
    ...
    with open(path, newline="") as f:
        for row in csv.DictReader(f):
            ...
            e_ana.append(float(row["E_analytic"]))
            e_num.append(float(row["E_kraken"]))
    return step, e_ana, e_num


def main():
    ap = argparse.ArgumentParser(description=__doc__)
    ap.add_argument("-c", "--csv", default=str(HERE / "taylor_green.csv"),
                    help="input CSV (default: taylor_green.csv next to this script)")
    ap.add_argument("-o", "--out", default=str(HERE / "taylor_green_repro.png"),
                    help="output PNG path")
    args = ap.parse_args()

    step, e_ana, e_num = load_csv(pathlib.Path(args.csv))
    ...
    ax.plot(step, e_ana, "-", color=line_c, lw=2.0, label="Analytical", zorder=1)
    ax.plot(step, e_num, "o", color=mark_c, mec="0.92", mew=1.0, ms=7,
            ls="none", label="Kraken", zorder=2)

    ax.set_xlabel(r"Time step")
    ax.set_ylabel(r"Mean kinetic energy")
    ax.set_title("Taylor-Green vortex decay — $N = 64$")
