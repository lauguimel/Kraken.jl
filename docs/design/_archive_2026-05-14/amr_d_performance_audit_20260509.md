...
### 2026-05-10 Hot-Loop Allocation Pass

The direct-route compact loop was still allocating when called through the
large scheduler function, even though the isolated SoA loop allocated nothing.
The fix was to split the flat direct route execution and channel boundary
reflection execution into narrow helper functions over precomputed arrays.

Local post-warmup measurements on the central nested4 Poiseuille case:

- one isolated transport step: `0.0214 s`, `907.9k` allocations, `15.3 MiB`
  before the split;
- one isolated transport step: `0.00056 s`, `2` allocations, `1.1 KiB` after
  the split;
- 200-step macro runner after warmup: about `0.22 s`, back in line with the
  previous optimized debug-loop target.

This pass also compiles boundary routes into per-level SoA arrays so the CPU
path and future GPU route-native kernels can share the same topology contract.

The temporal dashboard runner now also caches dense Cartesian channel
references within a run. Nested x-band/y-band/wall-yband cases with the same
flow, domain, level, physical time, and forcing reuse the same reference state
instead of recomputing the expensive resolved-grid baseline for every
refinement layout.

For diagnostics, the runner also exposes `KRK_AMR_D_TEMP_MASS_GUARD_RTOL` so a
dashboard can be completed with an explicit guard override while still
exporting `max_raw_mass_rel_drift` in `values.csv`/`validation.csv`.
`KRK_AMR_D_TEMP_REQUIRE_TEMPORAL=false` separates R2/tfinal validation from the
doubling-based temporal convergence gate for long fixed-time comparisons; the
CSV still reports `temporal_ok` and whether the gate was required.

### 2026-05-10 Active-Cell Dashboard Gate

The temporal dashboard now writes both raw leaf-field R2 and active-cell R2.
The validation gate uses active-cell R2: Cartesian and analytic references are
volume-averaged over each AMR active cell before comparison. This is the fair
metric for a conservative tree, where coarse cells intentionally represent
larger leaf-equivalent regions. Raw leaf-field R2 remains in `values.csv` as a
diagnostic for visual/blockiness artifacts.

Validated CPU/F64 nested channel dashboard:

```text
benchmarks/results/quicklook/amr_d_nested_closed_active_r2_tfinal_cpu_f64_12800_20260510
```

All four nested debug cases are `validated` and `converged` at `12800` AMR
steps with `tfinal_ok=1`.
...
- subcycling 2D: `679 pass`, `4 broken`
- AMR-D KRK validation 2D: `87 pass`
- GPU route pack 2D: `4984 pass`
- AMR-D quicklook artifacts: `28 pass`

## Remaining Bottlenecks

The next performance pass should focus on the remaining precomputed execution
arrays and device kernels:

- C2F application and child injection arrays;
- collision kernels over active level rows;
