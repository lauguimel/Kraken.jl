#!/usr/bin/env python3
r"""Reproduce the Kraken.jl example-08 Rayleigh-Benard temperature field figure.

Reads ``rayleigh_benard.csv`` (tidy columns: x, y, T) sitting next to this
script, pivots it back to a 2-D lattice and draws the temperature heatmap (the
SAME field the committed ``rayleigh_benard_temperature.svg`` plots), on the dark
#1b1b1f Vitepress page background. Ra = 5000, Pr = 1.

    python rayleigh_benard.py               # -> rayleigh_benard_repro.png
    python rayleigh_benard.py -o /tmp/r.png # custom output path
...
Env (Kraken docs): conda run -n kraken-v0-3-figures python rayleigh_benard.py
"""
...
DARK = "#1b1b1f"
T_COLD, T_HOT = 0.0, 1.0  # colour range matches the committed figure
...
def load_field(path: pathlib.Path):
    """Pivot the tidy (x, y, T) CSV into a Nx x Ny array + axis vectors."""
    xs, ys, vs = [], [], []
    with open(path, newline="") as f:
        for row in csv.DictReader(f):
            xs.append(int(float(row["x"])))
            ys.append(int(float(row["y"])))
            vs.append(float(row["T"]))
...
def main():
    ...
    ap.add_argument("-c", "--csv", default=str(HERE / "rayleigh_benard.csv"),
                    help="input CSV (default: rayleigh_benard.csv next to this script)")
    ap.add_argument("-o", "--out", default=str(HERE / "rayleigh_benard_repro.png"),
    ...
    pcm = ax.pcolormesh(x, y, field, cmap="inferno", shading="auto",
    ...
    ax.set_xlabel(r"$x$")
    ax.set_ylabel(r"$y$")
    ax.set_title("Temperature — $Ra = 5000$, $Pr = 1$")
