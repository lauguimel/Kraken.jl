#!/usr/bin/env python3
r"""Reproduce the Kraken.jl example-02 Couette velocity-profile figure.

Reads ``couette.csv`` (columns: y, u_analytic, u_kraken) sitting next to this
script and plots the analytical linear profile (line) against the Kraken LBM
solution (markers), on the dark #1b1b1f Vitepress page background.

    python couette.py                  # -> couette_repro.png next to csv
    python couette.py -o /tmp/c.png    # custom output path
...
Env (Kraken docs): conda run -n kraken-v0-3-figures python couette.py
"""
...
def main():
    ...
    ap.add_argument("-c", "--csv", default=str(HERE / "couette.csv"),
                    help="input CSV (default: couette.csv next to this script)")
    ap.add_argument("-o", "--out", default=str(HERE / "couette_repro.png"),
    ...
    ax.set_title("Couette flow — $N_y = 32$")
