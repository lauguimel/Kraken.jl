#!/usr/bin/env python3
r"""Reproduce the Kraken.jl example-12 reversed-vortex VOF field figure.

Reads ``reversed_vortex.csv`` (tidy columns: x, y, C) sitting next to this
script, pivots it back to a 2-D lattice and draws the volume-fraction heatmap
(the final ``C`` field after the full reversal cycle t = T), on the dark
#1b1b1f Vitepress page background.

    python reversed_vortex.py               # -> reversed_vortex_repro.png
    python reversed_vortex.py -o /tmp/r.png # custom output path
...
Env (Kraken docs): conda run -n kraken-v0-3-figures python reversed_vortex.py
"""
...
def main():
    ...
    ap.add_argument("-c", "--csv", default=str(HERE / "reversed_vortex.csv"),
                    help="input CSV (default: reversed_vortex.csv next to this script)")
    ap.add_argument("-o", "--out", default=str(HERE / "reversed_vortex_repro.png"),
    ...
    ax.set_xlabel(r"$x$")
    ax.set_ylabel(r"$y$")
    ax.set_title("Reversed vortex — recovered VOF field at $t = T$")
