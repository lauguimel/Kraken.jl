...
@testset "Conservative tree topology 2D" begin
    ...
    @testset "coarse to fine routes reproduce patch split operators" begin
        ...
        coarse_to_fine_patch_boundary_F_2d!(expected, coarse_src, patch)
    ...
    @testset "interface route primitives are explicit" begin
        ...
        @test all(route.kind == SPLIT_FACE for route in west_routes)
        @test all(isapprox(route.weight, 0.5; atol=0, rtol=0) for route in west_routes)
        @test sort(collect((topology.cells[route.dst].i, topology.cells[route.dst].j)
                           for route in west_routes)) == [(5, 9), (5, 10)]
        ...
        @test corner_routes[1].kind == SPLIT_CORNER
        @test corner_routes[1].weight == 1.0
        @test (topology.cells[corner_routes[1].dst].i,
               topology.cells[corner_routes[1].dst].j) == (5, 7)
