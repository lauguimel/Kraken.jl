...
@testset "FVFD operator library 2D" begin
    ...
    @testset "q_wall lowering ignores pure halfway and averages curved normals" begin
        ...
        if isdefined(Kraken, :precompute_q_wall_cylinder)
    ...
    @testset "embedded tensor divergence balances constant stress on q_wall cuts" begin
        ...
        if isdefined(Kraken, :logfv_polymer_force_embedded_bc_aware_2d!)
            fill!(fx, NaN)
            fill!(fy, NaN)
    ...
    @testset "coherent half-plane embedded geometry balances constant stress globally" begin
        ...
        if isdefined(Kraken, :logfv_embedded_wall_traction_2d!)
            fill!(tx, NaN)
            fill!(ty, NaN)
    ...
    @testset "embedded face velocity lowering applies q_wall aperture fractions" begin
        ...
        if isdefined(Kraken, :logfv_cell_velocity_to_faces_embedded_2d!)
    ...
    @testset "embedded advection wrapper preserves constants through q_wall apertures" begin
        ...
        fill!(ux_face, NaN)
        fill!(uy_face, NaN)
        if isdefined(Kraken, :logfv_advect_upwind_embedded_2d!)
    ...
    if isdefined(Kraken, :logfv_cell_velocity_to_faces_bc_aware_2d!) &&
            isdefined(Kraken, :logfv_advect_upwind_bc_aware_2d!)
    ...
    if isdefined(Kraken, :logfv_velocity_gradient_bc_aware_2d!)
