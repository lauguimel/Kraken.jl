...
#
# 1. `compute_polymeric_force_3d!` at non-periodic x and z faces must use the
#    same second-order one-sided difference as the y walls. A clamped central
#    difference returns (τ₂ − τ₁)/2 there: half the derivative, first order.
...
function bs3_polymer_force(periodic_x, periodic_z; Nx=8, Ny=7, Nz=6)
    ...
    compute_polymeric_force_3d!(F..., tau...; periodic_x=periodic_x, periodic_z=periodic_z)
    ...
    return F, (Fx_exact, Fy_exact, Fz_exact)
end
...
@testset "FVFD 3D boundary stencils" begin
    @testset "polymer force, non-periodic x and z faces" begin
        F, Fexact = bs3_polymer_force(false, false)
        for c in 1:3
