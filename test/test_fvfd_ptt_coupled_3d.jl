...
# ---------------------------------------------------------------------
# PTT (Phan-Thien–Tanner) 3D log-conformation COUPLED canary — BOTH FVFD
# drivers.
#
# Scope: the PTT coupling wired through the shared constitutive-step
...
# Mirror of the FENE-P / Giesekus coupled canaries, adapted to the PTT
# signature (extensibility ε + linear/exponential variant). NOTE:
# ν_eff = ν_total is NOT asserted — PTT shear-thins, so the effective
# polymer viscosity is rate-dependent; the dispatch is pinned instead by the
# ε=0 ≡ OB byte-identity safety net + the finite-ε signature.
#
# Checks (per driver):
#   P0/E0  ε=0 ≡ OB coupled, BYTE-FOR-BYTE (dispatch-correctness net).
#   P1     finite-ε shear-thinning: reduced peak N1 vs OB; NaN-free
#          (linear AND exponential variants).
#   E1     finite-ε bounded extension: center C_xx < OB(≈2) − tol; NaN-free.
#   K      poiseuille_ptt.krk + extensional_ptt.krk dispatch+run.
#
...
@testset "FVFD 3D PTT coupled (both drivers)" begin
    ...
    @testset "Poiseuille driver" begin
        ...
        @testset "P0 ε=0 ≡ OB coupled byte-identical" begin
            res0 = run_p(LogConfPTT(G=Gmod, λ=FT(λ), ε=0.0, variant=:linear))
            ...
            println("P0 PTT ε=0 vs OB: max|ΔΨ|sum=$(dpsi) max|Δu|=$(du)")
            @test dpsi == 0.0
            @test du == 0.0
        end

        @testset "P1 finite-ε shear-thinning (reduced N1 vs OB)" begin
            N1_ob = peak_N1(res_ob)
            @test N1_ob > 0
            for variant in (:linear, :exponential)
                ε = 0.2
                res_pt = run_p(LogConfPTT(G=Gmod, λ=FT(λ), ε=FT(ε), variant=variant))
                @test res_pt.completed_steps == max_steps
                @test all(isfinite, res_pt.ux)
                @test all(isfinite, res_pt.psi_xx) && all(isfinite, res_pt.psi_xy)
                @test all(isfinite, res_pt.psi_yy) && all(isfinite, res_pt.psi_zz)
                @test all(isfinite, res_pt.tau_p_xx) && all(isfinite, res_pt.tau_p_xy)
                N1_pt = peak_N1(res_pt)
                println("P1 PTT $(variant) ε=$(ε): N1=$(N1_pt) < OB N1=$(N1_ob) ?")
                @test N1_pt < N1_ob
            end
        end
    end
    ...
    @testset "Extensional driver" begin
        ...
        @testset "E0 ε=0 ≡ OB coupled byte-identical" begin
            res0 = run_e(LogConfPTT(G=Gmod, λ=FT(λ), ε=0.0, variant=:linear))
            @test res0.completed_steps == max_steps
            ...
            dpsi = maximum(abs.(res0.psi_xx .- res_ob.psi_xx)) +
                   maximum(abs.(res0.psi_yy .- res_ob.psi_yy))
            println("E0 PTT ε=0 vs OB: max|ΔC|sum=$(dc) max|ΔΨ|sum=$(dpsi)")
            @test dc == 0.0
            @test dpsi == 0.0
        end

        @testset "E1 finite-ε bounded, reduced stretch (< OB)" begin
            ε = 0.2
            res_pt = run_e(LogConfPTT(G=Gmod, λ=FT(λ), ε=FT(ε), variant=:linear))
            @test res_pt.completed_steps == max_steps
            @test finite_all(res_pt)
            cxx_pt = center_mean(res_pt.C_xx)
            cxx_ob = center_mean(res_ob.C_xx)
            println("E1 PTT ε=$(ε): Cxx=$(cxx_pt) < OB Cxx=$(cxx_ob)≈$(Cxx_ref) ?")
            @test cxx_ob > 1.5
            @test cxx_pt < cxx_ob
            @test cxx_pt < Cxx_ref - 2.0e-3
            @test cxx_pt > 1.0
        end
    end

    # ---- .krk name-dispatch + run (both drivers) --------------------
    @testset "K .krk PTT dispatch+run" begin
        ...
        cd(root) do
            ...
            println("K poiseuille_ptt: peak N1=$(maximum(abs.(rp.N1_prof)))")
            ...
            # PTT signature: bounded, reduced stretch below the OB value 2.0.
            @test 1.0 < cxx < 2.0
            println("K extensional_ptt: Cxx=$(cxx) (< OB 2.0)")
        end
    end
end
