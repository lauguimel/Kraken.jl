...
if _METAL_OK
    ...
    @warn "Metal not available — falling back to CPU F64 (NOT recommended for M22)"
    using KernelAbstractions
...
const BSD_FRACTION = 0.75; const BETA = 0.5
...
case_csv_path(R, wi) =
    joinpath(SCRATCH_DIR, "cyl_baseline_R$(R)_wi$(wi_output_tag(wi)).csv")
...
function print_summary(row)
    @printf(
        "M22 R=%d Wi=%s bsd=0.75: Cd_kraken=%s, Cd_rheotool=%s, err=%s%%, min_detC=%s, min_ceig=%s, u_max=%s, nan_flag=%s, dt=%.1fs\n",
        row[:R],
        wi_input_tag(row[:Wi]),
        ...
        fmt_nan(row[:min_c_eig], "%.4g"),
        fmt_nan(row[:u_max_abs], "%.4g"),
        string(row[:nan_flag]),
        Float64(row[:dt_s]),
    )
end
...
function main()
    ...
    println("M22 mode: $(mode == :full ? "full" : "selftest")")
    mkpath(SCRATCH_DIR)
    cases = mode == :full ?
