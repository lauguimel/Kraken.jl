...
if _METAL_OK
    ...
    @warn "Metal not available — falling back to CPU F64 (NOT recommended for M24A_bsd1)"
    using KernelAbstractions
...
const BSD_FRACTION = 1.0; const BETA = 0.5
...
case_csv_path(R, wi) =
    joinpath(SCRATCH_DIR, "cyl_v2_bsd1_R$(R)_wi$(wi_output_tag(wi)).csv")
...
function print_summary(row)
    @printf(
        "M24A_bsd1 R=%d Wi=%s bsd=1.0: Cd_kraken=%s, Cd_rheotool=%s, err=%s%%, min_detC=%s, trace_C_max=%s, trace_C_mean=%s, N1_max=%s, N1_mean=%s, nan_flag=%s, dt=%.1fs\n",
        row[:R],
        wi_input_tag(row[:Wi]),
...
function main()
    ...
    println("M24A_bsd1 mode: $(mode == :full ? "full" : "selftest") backend=$(backend_label) FT=$(FT) bsd=$(BSD_FRACTION)")
    mkpath(SCRATCH_DIR)
    cases = mode == :full ?
